...
struct InitialSetupWizard {
    ...
    func run() async throws {
        ...
        prompter.writeMessage("Saved configuration to \(input.repoRoot)/.nsassets.json")
    }
    ...
    private func confirmConfigReplacementIfNeeded(at repoRoot: String) throws {
        ...
        let shouldReplace = try prompter.promptConfirmation(
            ".nsassets.json already exists. Replace it?",
            defaultValue: false
        )
    ...
    private func updateGitignoreIfNeeded(at repoRoot: String) throws {
        let shouldUpdateGitignore = try prompter.promptConfirmation(
            "Add nsassets-artifacts/ to .gitignore?",
            defaultValue: true
        )
        ...
        if gitignoreContainsNSAssetsArtifacts(existingContents) {
            prompter.writeMessage("NSAssets artifacts are already ignored in \(gitignorePath)")
            return
        }

        try fileSystem.writeFile(appendedGitignoreContents(existingContents), toPath: gitignorePath)
        prompter.writeMessage("Updated \(gitignorePath) to ignore nsassets-artifacts/")
    }

    private func gitignoreContainsNSAssetsArtifacts(_ contents: String) -> Bool {
        contents
            .components(separatedBy: .newlines)
            .map { $0.trimmingCharacters(in: .whitespaces) }
            .contains("nsassets-artifacts/")
    }

    private func appendedGitignoreContents(_ existingContents: String) -> String {
        guard !existingContents.isEmpty else {
            return "nsassets-artifacts/\n"
        }

        let separator = existingContents.hasSuffix("\n") ? "" : "\n"
        return existingContents + separator + "nsassets-artifacts/\n"
    }
}
