...
struct AppBridgeTests {
    ...
    func doesNothingInInteractiveModeWhenUserDeclines() {
        ...
        #expect(outputLines.contains("Aperture Studio is not running. Skipping notification."))
    }
    ...
    func launchFailureSkipsNotificationAndEmitsDiagnostic() {
        ...
        #expect(poster.invocations.isEmpty)
        #expect(outputLines.contains { $0.contains("Failed to open Aperture Studio") })
    }
...
private let samplePayload = AppNotificationPayload(
    ...
    projectName: "MyApp",
    artifactPath: "/repo/aperture-artifacts/xcresults/Snapshots.json",
