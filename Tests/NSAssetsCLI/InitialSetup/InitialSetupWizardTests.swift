...
struct InitialSetupWizardTests {
    ...
    func replacesExistingConfigAndContinuesSetup() async throws {
        ...
        #expect(prompter.messages.contains("Saved configuration to /repo/.nsassets.json"))
        ...
        #expect(
            fileSystem.createDirectoryOperations.contains {
                $0.path == "/repo/nsassets-artifacts/logs" && $0.withIntermediateDirectories
            }
        )
    }
...
private func assertNormalizedSetupResult(_ context: NormalizedSetupContext) throws {
    ...
    #expect(context.prompter.messages.contains("Saved configuration to /repo/.nsassets.json"))
